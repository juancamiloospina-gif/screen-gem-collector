...
import { crearCaso } from "@/services/casos";
import { catalogoOperativoQuery, type CatalogoOperativo } from "@/services/catalogo";
...
function detectar(texto: string, catalogo: CatalogoOperativo): Deteccion {
  const t = texto.toLowerCase();
  const placaMatch = texto.toUpperCase().match(/([A-Z]{3})[\s-]?(\d{3})/);
  const placa = placaMatch ? `${placaMatch[1]}-${placaMatch[2]}` : "SIN-000";

  const ciudad =
    catalogo.ciudades.find((c) => t.includes(c.nombre.toLowerCase().normalize("NFC")))?.nombre ??
    catalogo.ciudades[0]?.nombre ??
    "Bogotá";

  const servicioGrua = catalogo.servicios.find((s) => s.nombre === "Grúa de gran tonelaje")?.nombre;
  const servicioJuridico = catalogo.servicios.find((s) => s.nombre === "Asistencia jurídica")?.nombre;
  const servicioTaller = catalogo.servicios.find((s) => s.nombre === "Carro taller")?.nombre;
  const servicioRescate = catalogo.servicios.find((s) => s.nombre === "Rescate")?.nombre;
  const servicioMovilidad = catalogo.servicios.find((s) => s.nombre === "Movilidad del conductor")?.nombre;

  let tipo_servicio: string = servicioGrua ?? catalogo.servicios[0]?.nombre ?? "Grúa de gran tonelaje";
  if (/abogad|jurídic|juridic|comparendo|tránsito|transito/.test(t) && servicioJuridico) tipo_servicio = servicioJuridico;
  else if (/batería|bateria|llanta|pinch|taller|no enciende|arranc/.test(t) && servicioTaller)
    tipo_servicio = servicioTaller;
  else if (/volc|atasc|rescat|barranco|hundi/.test(t) && servicioRescate) tipo_servicio = servicioRescate;
  else if (/taxi|hotel|hosped|movilidad|traslado del conductor/.test(t) && servicioMovilidad)
    tipo_servicio = servicioMovilidad;
  else if (/camión|camion|tractomula|mula|grúa|grua|pesado/.test(t) && servicioGrua)
    tipo_servicio = servicioGrua;

  const tipo_vehiculo = /camión|camion|tractomula|mula|pesado|furgón|furgon/.test(t) ? "Pesado" : "Liviano";

  const prometido_min =
    catalogo.servicios.find((s) => s.nombre === tipo_servicio)?.prometido_min ?? 45;

  const ubicacion = texto.replace(/^.*?(en|vía|via)\s/i, "").trim().slice(0, 120);
...
function Reportar() {
  ...
  const { data: catalogo, isLoading: loadingCatalogo } = useQuery(catalogoOperativoQuery);
  ...
  function enviar(valor: string) {
    ...
    if (!catalogo || loadingCatalogo) {
      agregar({
        de: "ia",
        texto: "Estoy cargando la configuración operativa desde Supabase. Inténtalo de nuevo en unos segundos.",
        hora: ahoraHora(),
      });
      return;
    }

    const d = detectar(limpio, catalogo);
    setTimeout(() => {
      agregar({
        de: "ia",
    ...
    <main className="p-4 lg:p-7">
      <div className="mb-6">
        ...
        <p className="mt-1 text-xs text-ops-muted">
          El conductor describe lo ocurrido y el agente de IA estructura el caso.
        </p>
      </div>
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
        <section className="flex min-w-0 flex-col overflow-hidden rounded-xl border border-ops-line bg-ops-navy">
          <div className="flex h-16 items-center justify-between border-b border-ops-line px-5">
            <span className="flex items-center gap-3 text-sm font-bold">
              ...
              Agente de asistencia AssisPrex
            </span>
          ...
          <div className="flex h-[58vh] min-h-[480px] flex-col gap-4 overflow-y-auto bg-ops-deep/40 p-5">
            {mensajes.map((m, i) => (
              <div key={i} className={`flex ${m.de === "conductor" ? "justify-end" : "justify-start"}`}>
                <div
                ...
                >
                  ...
                  {m.texto}
                  <div className="mt-1 text-right font-mono text-[9px] tabular-nums text-faint">{m.hora}</div>
                </div>
              </div>
            ))}
            <div ref={finRef} />
          ...
          {pendiente && (
            <div className="flex flex-wrap items-center gap-3 border-t border-ops-line bg-ops-panel/60 px-5 py-4">
              <span className="text-[11px] text-muted-ink">Confirmar apertura del caso para {pendiente.placa}</span>
              <Button onClick={confirmar} disabled={creando} size="sm" className="ml-auto rounded-lg font-bold">
          ...
          >
            ...
            />
            <Button type="submit" size="icon" className="size-11 rounded-lg" aria-label="Enviar mensaje">
              <Send />
            </Button>
          </form>
        </section>
        ...
        <aside className="overflow-hidden rounded-xl border border-ops-line bg-ops-navy">
          ...
          <p className="px-5 py-4 text-[10px] leading-relaxed text-ops-muted">
            Demo: el agente de IA responde con un guion preparado. La integración con WhatsApp Business,
            voz y aseguradora está simulada en este prototipo.
          </p>
        </aside>
      </div>
    </main>
